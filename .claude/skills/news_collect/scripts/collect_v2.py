...
def setup_notebooklm_notebook():
    ...
    try:
        ...
        if check.returncode != 0:
            err = (check.stderr or '').strip() or (check.stdout or '').strip() or '未知错误'
            print(f"   ⚠️ NotebookLM 命令不可用: {err}")
            return False
        ...
        print(f"   ⚠️ NotebookLM 设置失败: {type(e).__name__}: {e}")
        return False
...
def upload_to_notebooklm(markdown_content, title):
    ...
    try:
        ...
        temp_file = Path(tempfile.gettempdir()) / f'{upload_date}_{safe_title}.md'
        ...
        if result.returncode == 0:
            ...
            err = (result.stderr or '').strip() or (result.stdout or '').strip() or '未知错误'
            print(f"   ⚠️ 上传失败: {err}")
            return False
    except Exception as e:
        print(f"   ⚠️ 上传失败: {type(e).__name__}: {e}")
        return False
