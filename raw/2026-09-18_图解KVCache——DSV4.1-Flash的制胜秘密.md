...
**来源**: https://waytoagi.feishu.cn/wiki/Ny3awPIcWig9bCkQdLYcHbabnKd
...
> 🔗 原文链接：[https://x.com/wquguru/status/20991197602...](https://x.com/wquguru/status/2099119760248389730)

> *WquGuru · X · @wquguru · 2026-09-13 20:55*

> *📌 本文为转载，版权归原作者所有，全文以原文链接为准*

![图片展示了DS V4.1-Flash技术在KV Cache压缩方面的成果。柱状图对比了V1、V3.2、V4-Flash和V4.1-Flash四个版本的KV Cache大小，其中V1为380 KB，V3.2为47 KB，V4-Flash为3.4 KB，V4.1-Flash仅为890 B。右上角标注了2023.11到2026.09的倍数变化为437倍。该图与上下文紧密相关，直观呈现了DS V4.1-Flash技术在KV Cache压缩方面的显著进步，有效解决大KV Cache持续挤占HBM和SSD容量与传输带宽的问题。](https://feishu.cn/file/Dd7UbKoyBoBLoDxt24KcKLstnIe)
...
![图片展示了四代DeepSeek中每token的global KV大小变化情况。从V1 67B的380 KB，到V3.2的47 KB，再到V4-Flash的3.4 KB，最后到V4.1-Flash的890 B。其中，V4.1-Flash的KV大小为890 B，是V1 67B的437倍。该图与上下文紧密相关，直观呈现了四代DeepSeek中KV大小的显著变化，为理解KV Cache技术进步提供数据支撑。](https://feishu.cn/file/OrzHbtfnFonxYGxV6O3cbMS3nZA)
...
![图片展示了KV Cache在生成文本时的计算效率对比。左侧“没有KV Cache”示例，每生成一个字需重算所有字的K、V，计算量为O(N²)；右侧“有KV Cache”示例，算过K、V存进显存，只算新字，计算量为O(N)，且每个字层都要存一份K、V。图片与上下文紧密相关，直观呈现了KV Cache通过缓存K、V向量，减少计算量，提升生成文本效率的原理。](https://feishu.cn/file/ElYgbeYzXoCZtsxoIMhcukASn6A)
...
![图片以DeepSeek-V1 67B模型为例，展示了KV Cache的计算公式及大小。公式为：95层 × 2个KV × 8个KV头 × 128维 × 2字节 = 每token 389 KB。图中还呈现了不同上下文长度下KV Cache的大小，如4K上下文为1.5 GB，32K上下文为11.9 GB，128K上下文为47.5 GB，1M上下文为380.0 GB。该图与上下文紧密相关，直观说明了KV Cache的计算方法及随上下文长度增加而线性增长的特点。](https://feishu.cn/file/RRhYbZJhuoywpIxWHivcBaaXntd)
...
![图片展示了GPU内存的金字塔结构，从上至下依次为片上SRAM、HBM显存、主机内存DRAM和SSD。片上SRAM容量几十MB，带宽几十TB/s，只放正在计算的一小块KV。HBM显存容量80 - 192 GB/卡，带宽3 - 8 TB/s，存runtime KV、global KV等。主机内存DRAM容量1 - 2 TB/台，带宽几十几百GB/s，存persistent KV等。SSD容量几十TB/台，带宽几GB/s，存persistent KV等。该图与上下文紧密相关，直观呈现了不同层级内存的容量、带宽及存储内容，帮助理解为何显存如此金贵。](https://feishu.cn/file/XyuxbwgeroE8BBxq5nJc7eHRnIh)
...
![图片展示了GPU封装的剖面图，说明算力die和HBM显存并排住在同一块中介层上。图中HBM堆叠显存由4层DRAM die垂直堆叠而成，GPU计算die位于其一侧，逻辑die（Logic Die）位于下方，硅中介层（Interposer）连接各部分。TSV硅通孔是带宽来源，连接各die。底部标注HBM容量计算公式及带宽情况，强调HBM容量受限于封装面积和面积限制，且只能加堆或加层。](https://feishu.cn/file/SPnDbXuZXoina0xxGgccVAqPnse)
...
![图片展示了KV Cache的三个乘法维度，即每条KV有多大（entry大小）、存多少条（序列长度）、存几份（层数）。每个维度用不同颜色标注，如entry大小用紫色，序列长度用蓝色，层数用橙色。图中还说明KV Cache总量为这三个维度的乘积。底部文字强调盒子的体积即驻留显存，稀疏注意力每步只读盒子的一小部分，省的是带宽和算力，不缩小盒子，要腾出显存放下一个请求，必须缩小盒子本身。该图与上下文紧密相关，是对KV Cache突破点的直观呈现。](https://feishu.cn/file/UYYmbAvULoB1vkxTEJAcGPrwnYf)
...
![图片展示了KV Cache在DS V4.1 - Flash中六大突破。突破一为GQA，2023年Llama 2/DeepSeek - V1中K、V头数从8降到2，缓存减少四分之一。突破二为MLA，2024年DeepSeek - V2中K、V压成一个latent存，每层2048个存576个。突破三为量化，V3 FP8到V4.1 FP4，一个数从2字节到半字节，存储减少四分之一。突破四为滑窗+稀疏/压缩，V3.2 DSA到V4 CSA，滑窗128、全量512，CSA每2个token压1条。突破五为跨层复用，CLA / YoCo到V4.1 CSA2，存KV、借KV，多层共用一份KV，40层存4份。突破六为系统层，PagedAttention、分层存储，按需分配、三层分级存储不改模型，改部署。](https://feishu.cn/file/UiEab58Mro9ql7xB3YicuDponoh)
...
![图片展示了从MHA多头注意力到GQA分组查询注意力再到MLA多头潜在注意力的KV缓存压存过程。MHA每个Q头配一组K、V，缓存8组K+8组V共2048个数/层；GQA4个Q头共用一组K、V，缓存2组K+2组V共512个数/层；MLA所有头共用一个低秩latent，缓存1个latent+64维RoPE共576个数/层。MLA关键在于K、V不再分头存，只存一份低秩“潜在向量”，DeepSeek-V2将每tokenKV从V1的389KB压到约70KB，还保住了MHA的精度。](https://feishu.cn/file/DXaRbGwFBo89vyxEPwcc4F2wnMe)
...
![图片展示了CED（因果编码器 - 解码器）架构，说明上半段模型的KV由下半段代算。图中分因果编码器和解码器两部分，解码器20层的KV由第20层输出乘各层独立投影矩阵得到，而非各层自己算。解码器的KV为H520 × 各层独立投影矩阵，且只跑Encoder 20层，Decoder的global KV直接投影得到激活参数8B。而Decode（生成）阶段，40层全推，激活参数16B，读的是同一份global KV。](https://feishu.cn/file/HDXSbYs5uomgKfx00iVcBb6xnxh)
...
![](https://feishu.cn/file/N1Iyb7GdloQdqzxOEZ4c1EDlnmg)
...
![](https://feishu.cn/file/X9EubWK1Wol5u9xMw4JcZOxJnMU)
...
![](https://feishu.cn/file/RnK5byYEloaJT5x739ictBInnIb)
...
![](https://feishu.cn/file/IRGAbEfIWoqM7exSl8qcknRjnZf)
