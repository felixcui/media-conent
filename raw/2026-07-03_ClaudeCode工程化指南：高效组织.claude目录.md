...
本文介绍了Claude Code中高效组织.claude/目录的工程化指南，旨在解决项目规模扩大后配置混乱的问题。文章提出了一套标准目录结构蓝图，并阐述了三大核心原则：一是顶层保持轻量，区分负责引导的CLAUDE.md与负责控制的settings.json，并支持本地覆盖；二是合理划分全局与专项指导，CLAUDE.md存放架构等核心信息，rules/按模块拆分具体规则；三是明确hooks/与com。
