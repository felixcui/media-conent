...
**来源**: https://mp.weixin.qq.com/s/PG-rXJllhrtynAfNZn5asg

---

## 摘要

Skill是AI世界里的“菜谱”，能将模糊需求转化为稳定可复用的AI执行单元，解决AI自由发挥导致的高成本与低确定性问题。它本质上是一个包含特定指令的文件夹，核心在于三级渐进式披露机制，即通过始终加载的元数据识别场景，按需加载的正文提供执行步骤，以及深度引用的参考资料实现精准读取。这种机制既节省上下文又能承载复杂知识，让用户无需反复解释，AI即可按标准稳定交付高质量结果，全面提升人机协同效能。
...
```sql
5
→ 打开终端，运行：    npx skills add from-design→ 选择 Qoder，选择 Global，选择 copy 模式
5
→ 在 Qoder Quest 模式中，输入一个前端设计需求→ 观察 AI 是否自动调用 from-design Skill→ 如果没有，输入 "/from-design" 手动调用
10
→ 打开 ~/.qoder/skills/from-design/SKILL.md→ 在 description 中加一句符合你实际场景的触发词→ 重启会话，再次测试
10
→ 在项目目录下：    mkdir -p .qoder/skills/my-first-skill    touch .qoder/skills/my-first-skill/SKILL.md→ 填写 name、description 和执行步骤→ git commit 提交，通知团队成员拉取
```
