# 吴恩达最新开源 AI ，在Github爆火了！

**作者**: Datawhale

**来源**: https://mp.weixin.qq.com/s/2C-6DvdAut5ECkQrAqnRZw
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/zW6S9vt0cS8RicPrdw4w6yDajjm4iagd40M6TWIxyibwpibbKN8hHjEhvOxh3dHLk0CQicJiauXbRLwY7JaibON5tFIGWNTYzpSCDHgSKic4OFNpwCI/640?wx_fmt=png&from=appmsg)

```bash
项目地址：https://github.com/andrewyng/openworker官网下载：https://openworker.com
```
...
```sql
┌────────────────────────────────────────────────┐│              OpenWorker desktop app            │  Tauri 2 + React 18├────────────────────────────────────────────────┤│           local agent server (Python)          │  FastAPI · 127.0.0.1:8765├───────────────┬────────────────┬───────────────┤│  your files   │   your tools   │  your model   │  25+ 集成 + MCP│  & terminal   │ 25+ connectors │  any provider │  30 个预置模型 + Ollama└───────────────┴────────────────┴───────────────┘
```
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/zW6S9vt0cS8wuZadSz9eF1W0tWb0rUwzwppmsw1ek9K8x9YakaLKFcFHR1msEERSGzMZ4k5kU5T7FfOptMEY76MUdl6ianzgzYIe0FcibYvPY/640?wx_fmt=png&from=appmsg)
...
```bash
git clone https://github.com/andrewyng/openworkercd openworker# 初始化环境（创建 .venv）bash packaging/setup_dev_env.sh# 启动本地 Agent 服务器.venv/bin/openworker-server --cwd ~/your/project --port 8765# 另开一个终端，启动 UIcd surfaces/guinpm installnpm run dev
```
