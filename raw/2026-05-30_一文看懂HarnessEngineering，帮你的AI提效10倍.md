...
**来源**: https://waytoagi.feishu.cn/wiki/Hrbsw06rWitZjYkZi7KcoKOYnQf

---

## 摘要

尽管顶尖大模型写代码能力出色，但在从零构建真实软件项目时完成率为零，暴露出缺乏架构和长期规划的短板。文章提出“Harness Engineering（马具工程）”概念，指出Agent等于模型加马具，强调应将Hooks、MCP等零散工具视为一个包含上下文管理、执行能力、任务编排、反馈机制和架构护栏五个核心维度的系统工程来设计，以此补足裸模型缺陷，实现AI编程效能的数倍提升。

---

## 正文

> 🔗 原文链接： [https://x.com/FakeMaidenMaker/statu...](https%3A%2F%2Fx.com%2FFakeMaidenMaker%2Fstatus%2F2055146731625447516)
...
OpenAI 公开过一个更夸张的案例。三个人的小团队，五个月时间，驱动 Codex 写出大约 100 万行代码，合并了约 1500 个 PR。这不是真正有内部日活用户、有外部 alpha 测试用户的软件产品。
...
它给你一份 检查清单 ：你的 AI 工作流在 5 个核心维度上是否完整？保证你的马匹永远处在最佳状态。
...
放在 AI 工程的演进脉络里看：
- 最内层是 提示词工程 ，关心怎么给 AI 下指令。
- 中间一层是 上下文工程 ，关心给 AI 什么信息、什么时候给。
- 最外层的 Harness 工程 把前两者都包在里面，再加上工具编排、状态持久化、验证循环、任务拆解、子 Agent、权限沙箱、回滚机制，构成一套完整的工程基础设施。
...
1. 再读 progress.md，看当前进度和未完成列表
1. 从未完成列表里挑当前优先级最高的一项
1. 完成这一项，跑验证，提交 git
1. 更新 progress.md，写清楚这一轮做了什么、下一轮该做什么
...
1. Chrome DevTools Protocol 接入 Agent runtime。 这让 AI 在浏览器层面拥有和工程师一样的调试能力：看 DOM 结构、监听网络请求、注入 JS、截图。复现一个 UI bug 不再需要工程师亲自打开浏览器，AI 自己就能复现并定位。
1. 日志、指标、trace 暴露给 AI 查询。 OpenAI 让 Codex 能直接用 LogQL、PromQL、TraceQL 这些查询语言访问生产监控数据。这意味着 AI 在排查问题时不需要工程师把日志手动贴给它，自己就能 grep 日志、看 metric 异常、追 trace。
1. Custom linter 把架构约束写成可执行规则。 OpenAI 把哪些层不能调用哪些层、哪些命名必须统一、哪些模式是禁止的，全部写成自动检查的 linter 规则。AI 写出的代码只要违反就被拦住。
1. 后台垃圾回收任务。 这是最有意思的一项。OpenAI 让后台定期跑一批 Codex 任务，扫描整个代码库找偏离架构原则的地方，自动开小 PR 来修。也就是说，技术债的偿还也被自动化了，不再依赖工程师手动重构。
...
# 结语：Harness Engineering 成为核心能力
所有大模型的 ProgramBench 全员 0% 这个结果反过来说明了一件事：
Harness Engineering 越来越重要。
以现在 AI 能帮你写掉一大部分代码，但 AI 不会替你做需求分析、不会替你拆任务、不会替你设计上下文架构、不会替你判断哪个工具该接哪个该删、不会替你建立验证标准、不会替你守住架构边界。
这些事情的总和就是 Harness Engineering。
它听起来像一个新名词，本质上是把传统软件工程里的方法学系统化地迁移到 AI 工作流上：
- AGENTS.md 对应 -> 需求文档和设计文档；
- 任务编排对应 -> 敏捷开发的迭代拆分；
- 反馈机制对应 -> 单元测试和 code review；
- 架构护栏对应 -> 代码规范和安全审查。
...
感谢您阅读到这里🙏
我会持续分享职场 x 副业 x 投资 x AI干货内容，感兴趣请点击关注，谢谢