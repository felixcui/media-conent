...
**来源**: https://mp.weixin.qq.com/s/D4RId9oiULoFj567vawPig

---

## 摘要

企业Agent从演示走向生产的核心难点不在于模型能力，而在于权限控制、经验复用与长任务状态管理等基础运行时问题。为避免“一场景一Agent”或“直接暴露编码Agent”等不可持续模式，企业应重构架构边界：以统一的Agent Harness作为完整运行时保障执行安全与状态恢复，以Skill机制沉淀和分发领域能力，结合虚拟文件系统管理任务上下文，实现稳定内核与可变领域能力的分离，让Agent真正稳定、。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr7haO8dA4BtptnAKnNpXzUosc9gdDevILhpRicX62EZxyz2ms5wLdThg93ov6X0aLq77sqbdu1HeAUj4ibMPWYmtLxK7l8F3NJKo/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr6SEsXLpyGUoVS18biatbn1cnRcQFwZiaYTW2LBlTqubebliaDkLreGVlKmlIHlsK3HSDpKr277rkgxfnklnIZS5NQI3t0tNMXzzY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr4KeVCGKBwn54mr7OcXJtMSnobOoKCibicFShcU19BCZticPej1Ig1guraKibcjHEEIKSNEpMiaibsW4gAGQdo7TabCZ7PtI7eFwZ5Z0/640?wx_fmt=png&from=appmsg)
...
```bash
/sessions/<session-id>/  scope.json          # 本会话绑定的项目、租户、环境和权限快照  evidence/           # 工具拉取的原始只读证据  working/            # 中间数据、脚本、计划与草稿  artifacts/          # 用户可消费的报告、图表、文档  checkpoints/        # 可恢复的执行状态或其索引  manifest.json       # 产物来源、hash、owner、审批与交付状态
/skills/              # 版本化 skill，可配置只读或受控写入/memories/            # 跨会话的稳定偏好和项目约定/shared/              # 显式发布后才能进入的团队共享资产
```
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr5nOKSWYrIVdQPDqZOicLfJAH5hhhh6KsJWmX79zxIKBYAMBqXibKDQVhyunPTefGnDKOEhEC97A8YJXh3mKtiazT425vW6ficuian8/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr4GKPINja3qVfjhGuv4oLF19rhPoS8qOW5PN7GQazV2S2HAQ3GuMgScenzWv4Px6XdgKU4cCicVw9MQqFnYRVh2nt9DCwlFFXWQ/640?wx_fmt=png&from=appmsg)
...
```python
class SessionContext(TypedDict):    # 创建会话后不可由模型修改    user_id: str    tenant_id: str    project_id: str    environment: str    capability_id: strclass AgentState(TypedDict):    # 可 checkpoint 的执行状态    messages: list    plan: list    selected_skills: list[str]    pending_approvals: list[str]class ArtifactState(TypedDict):    # 通过 VFS 保存的大对象与交付事实    evidence_paths: list[str]    artifact_paths: list[str]    manifest_path: str
```
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr67y3l6IIf9OK7MIHVqmTLtwXOicO6CsiarInhT4IzJDDCeZfUgIyicibCcCuVEiaWkMBphxp9Y2RIvlGKMzQR2szoClg23Tg5M9lBs/640?wx_fmt=png&from=appmsg)
