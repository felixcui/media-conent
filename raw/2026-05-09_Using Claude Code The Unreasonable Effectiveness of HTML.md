...
Anthropic Claude Code 团队工程师 Thariq 的长文，阐述为什么在 AI Agent 时代应该用 HTML 替代 Markdown 作为 Claude Code 的输出格式。核心观点：Markdown 的隐含假设是"人会从头读到尾"，而 HTML 的隐含假设是"人只想扫重点和动手改"。HTML 在信息密度、可视化清晰度、分享便捷性、双向交互等方面全面优于 Markdown，特别适合 spec 规划、代码审查、设计原型、研究报告和自定义编辑界面等场景。虽然 HTML 生成耗时是 Markdown 的 2-4 倍，但在 Opus 4.7 的 1MM context window 下代价可忽略，体验提升 10 倍。
