...
**来源**: https://mp.weixin.qq.com/s/Kgqi3XqTZSroyDCpUgsdsw

---

## 摘要

本文介绍了Anthropic关于AI Agent的Zero Trust（零信任）安全框架白皮书。由于AI将漏洞利用时间大幅缩短，且Agent具备无人值守执行、调用工具和自主决策能力，传统边界防御与摩擦型安全措施已失效。文章强调应遵循“永不信任、始终验证”、“假设已被攻破”和“最小权限”三大原则，优先选择移除攻击能力的控制而非仅增加麻烦。
...
![](https://mmbiz.qpic.cn/mmbiz_png/jXSGuwJvpdhuyepQNvjEuFMcmBBNXvEunODWkJsw8ibEcOzxKEbcXVPdsfaUo4SpBtIyOcxUqyMzH4BeWnYHkDUJILEqT14CXibseG7GFauOU/640?wx_fmt=png)
...
**无人值守执行** Agent 不需要人在每一步批准就能执行操作。一个做研究的 agent 可能搜索网页、综合信息、产出报告，全程没有人工审核。被操控的 agent 能以机器速度造成伤害

**工具访问** Agent 可以与 API、数据库、文件系统和外部服务交互。这包括 MCP（Model Context Protocol）。一个被攻破的 MCP 栈可能导致数据窃取、恶意代码执行和破坏

**决策能力** Agent 需要解读指令并选择如何完成目标。一条对人类来说看起来无害的指令，可能被 agent 解读为能实现完全不同结果的方式

**上下文持久化** Agent 可以在会话间维持记忆。记住之前的交互让 AI 助手更能干，但也产生了新的数据保护需求

**多 agent 协作** Agent 可以与其他 agent 通信。这种信任关系让攻击者可以攻破一个 agent 然后横向移动，到达初始目标无法直接访问的系统
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/jXSGuwJvpdh3MictDgNvCobiaxjX2r93TlPKYmNqSO6R3hkD4vvNRS3Z6oXQffwtVjP0Lvmh62ESSrXF1qdjVR4sCbWxX3ryaoicYdq0AHvKCQ/640?from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/jXSGuwJvpdhNjuAt0Od2quS1d9GNicEjAsry08k3nMQGCc4cyDGNC5y16SCec3u86Ag9sskdw9t2QCLLlQSQrhgafUACg0jEatdibZib9yLRqg/640?from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/jXSGuwJvpdia4J2pVYRlSW9iboMBqhetnOahicde6oVZhIcpSnhBZHGRBk4WPHNUWOHwCo1zBE16W0Om3NMJpKQpHrdUcfm3q0HscyJHzuLso8/640?from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/jXSGuwJvpdiaIzASq304DiaZXH95Jf9plcR3KMw2QvmjJIrFbD8lHMDw5icgE03pWy1cJG1C0wYVqMia4LttiajTsTocJMCbIfj5FlaD6kibjjbkM/640?from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/jXSGuwJvpdjY1rTD2icN71vbkcOetdYqicrK7k51XicL43EJr8pezQT7bmUxoAficSpjeiakpZWg3ibJAO78UibnWZMnIKyRlKwYgdP61wX2lmcSqo/640?from=appmsg)

输入验证与输出控制三级框架

输入清洗不能从传统技术直接搬到 agent 上。SQL 注入有明确定义的模式和受限的输入字段，但 agent 的输入是自由形式的

Advanced 级别增加了 **聚光灯技术（spotlighting）** ，利用已知 schema 帮助 LLM 区分系统指令和用户输入。 **宪法分类器（constitutional classifiers）** ，Anthropic 的方法在测试中阻止了 95% 的越狱尝试
...
![](https://mmbiz.qpic.cn/mmbiz_png/jXSGuwJvpdhnMttiaOickol28mcrk8fcM4H7t8GvmMHh8S4iaKicAPDHMaQam3C9ELPwdKzACFSreerE4bCXZuW5fx4dbzo1McaKEF5a2SGianhU/640?from=appmsg)
...
**分配唯一身份** 每个 agent 实例需要一个唯一的、有密码学根基的标识符。没有独立身份，在事件中关联日志就变成了猜测

**批准/禁止的动作** 文档化哪些动作被允许或禁止。一个被允许读取客户记录、总结信息和起草回复的 agent 有清晰的边界。一个有模糊权限「帮忙做客户服务」的 agent 没有
...
**工具白名单** 将 agent 限制在批准的工具内，默认拒绝。要在 agent 级别和 agent 外部两个层面控制。静态 API 密钥不适合用于工具认证，即使在 Foundation 级别

**能力限制** 限制允许的工具能做什么。邮件工具可能被限制为只能阅读，发送能力需要单独授权

**参数验证** 在执行前验证工具调用参数。参数验证应该在 agent 端和工具端都做

**沙箱执行** 带受限网络访问和系统调用过滤的容器沙箱遏制被攻破工具的影响。速率限制是摩擦不是屏障
...
**可解释性** 你能否将任何 agent 动作追溯到其触发输入并解释 agent 为什么选择那个响应

**行为一致性** 追踪 agent 动作是否与预期策略和模式一致。突然偏好不同工具的 agent 值得调查

**检测速度** 度量团队多快能意识到 agent 行为异常。对关键系统的目标是一小时内检测到
