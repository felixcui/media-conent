...
**来源**: https://mp.weixin.qq.com/s/3-hQ4vHYErfpIzYPC6wJwg
...
![](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1zjv2dIYFXghV8vbkVvyl6BAHGrUvuh5GicLeOvKibjlib1s7AqU76efA77ywhHBEzyyBkia42YjbB5YHo5ezRQc1GGQDlNQaNFSBw/640?wx_fmt=png&from=appmsg)
...
```js
本地 Vibe Coding 开发  → 推预发部署    → 人工在预发环境验证结果      → 人工把验证结果反馈给 AI        → AI 继续改          → 再次推预发 → 再次人工验证 → ...
```
...
```nginx
FilesystemService → OssStorageAdapter (具体类，直接调 OSS SDK)AgentWorkspace → SandboxCommandExecutor (具体类，调远程沙箱 API)
```
...
```cs
StorageAdapter (接口)  ├── OssStorageAdapter (线上，走 OSS SDK)  └── LocalStorageAdapter (本地，走 java.nio.file)CommandExecutor (接口)  ├── SandboxCommandExecutor (线上，调远程沙箱 API)  └── LocalCommandExecutor (本地，ProcessBuilder + bash -c)
```
...
```typescript
@Configuration@Profile("local")public class LocalRepositoryConfig {    @Bean    CommandExecutor localCommandExecutor() { ... }    @Bean("localFsBasePath")    String localFsBasePath() { ... }    // 用 AtomicLong 替代 TDDL GroupSequence    @Bean("sessionSequence")    Sequence sessionSequence() { return new LocalSequence(); }}
```
...
```typescript
public AgentRunner(@Nullable @Qualifier("localFsBasePath") String localFsBasePath, ...) {    // 线上环境：localFsBasePath = null，走 OSS 路径    // 本地环境：localFsBasePath = "/tmp/agentfs"，走本地路径}
```
...
| 低 | 类型上提 | 字段类型从实现类改为接口 |
| 中等 | 新增可选参数 | 构造器加可选的本地路径参数（原有构造器不动） |

线上代码路径中，这些新增代码全部被跳过：null 检查不通过、Profile 不匹配、接口多态自然走原有实现。一个检验标准：删掉所有本地相关代码后，线上行为完全不变。

**3.3 工具 AI 化：CLI 优先**

![](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1xEDaLAEubZWjianp506XdE4zx3wkY9x1uU7dRjAltSAeCKLB3xibW8CpGSiaWQXNibObYQXj7nF2h3OpmllhJhcbplfnhEMNmtHIU/640?wx_fmt=png&from=appmsg)
...
```css
# 通过 mw-cli 查询 Diamond 运行时配置值mw diamond get --unit online --data-id application.properties --group DEFAULT_GROUP# 通过 mw-cli 查询 HSF 服务地址mw hsf address --unit daily --app my-application
```
...
```cs
# fetch-switch-config.sh 核心逻辑mw diamond get idealab-agent-runtime asp-switch --env staging --unit pre -o json# → 解析 JSON，写入 switch-config-local.properties# → switch.modelApiKey=xxx# → switch.defaultTimeoutMs=60000# → switch.maxReactIterations=10
```
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1yf2r3tcIBB2NEoyEPYldnSZaBVj22icExf7pR4licMibLPfLyxdf72N4LJQN90JyMfV4b4Jtq8K2VClxUf8MgoKVZ0J2UWtlyUxs/640?wx_fmt=png&from=appmsg)
...
```markdown
# 项目简介AI Agent 运行时平台，支持 ReadFile/WriteFile/Bash 等 Tool。# 本地开发- 启动：\`mvn spring-boot:run -Dspring.profiles.active=local\`- 测试：\`mvn test\`- 本地文件系统根目录：\`/tmp/agentfs/\`# 架构约束- 上层模块只依赖接口，不依赖具体实现类- 本地专属代码通过 @Profile("local") 隔离- 新增本地适配不得修改线上代码路径
```
...
```bash
#!/bin/bash# 本地 Harness 验证脚本：AI 可以直接跑这个来确认本地环境是否正常set -eecho "=== 1. 编译检查 ==="mvn compile -q -Dspring.profiles.active=localecho "=== 2. 单元测试 ==="mvn test -q 2>&1 | tail -5echo "=== 3. 本地启动检查 ==="timeout 30 mvn spring-boot:run -Dspring.profiles.active=local &PID=$!sleep 15if curl -s http://localhost:8080/actuator/health | grep -q "UP"; then    echo "✓ 本地启动成功，health check 通过"else    echo "✗ 本地启动失败"    exit 1fikill $PID 2>/dev/nullecho "=== 4. 文件系统闭环检查 ==="# 验证 ReadFile/WriteFile 和 Bash 看到的是同一套文件TEST_FILE="/tmp/agentfs/test-$(date +%s)/verify.txt"mkdir -p $(dirname $TEST_FILE)echo "hello" > $TEST_FILEif [ -f "$TEST_FILE" ] && [ "$(cat $TEST_FILE)" = "hello" ]; then    echo "✓ 文件系统闭环正常"else    echo "✗ 文件系统闭环异常"    exit 1firm -rf $(dirname $TEST_FILE)echo "=== 全部通过 ==="
```
...
![](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1xFlFcLcFK6ibneDSdDaE3hPAufq0LaNlAQM3cPAAA8gDXuLOaKvGp8dYwSd3I3QIWZF9HBW06ClKdEqiaekgia8VY4pI21yxxl6A/640?wx_fmt=png&from=appmsg)
...
本地环境还可以再往前走一步，把 JVM 诊断能力也工具化。比如应用响应超时时自动跑 `jstack` 把结果输出到 AI 能读的地方，或者通过 Skill 封装 Arthas 的 `watch` 、 `trace` 、 `tt` 命令，让 AI 实时观测方法调用。异常信息如果能以 JSON 格式输出，AI 直接就能解析定位到具体代码行。
...
```objectivec
# application-local.propertiesspring.datasource.url=jdbc:h2:file:./data/local-db;AUTO_SERVER=TRUE;MODE=MySQLspring.datasource.driver-class-name=org.h2.Driverspring.datasource.username=saspring.datasource.password=
```
...
```java
public class LocalSequence implements Sequence {    private final AtomicLong counter = new AtomicLong(        System.currentTimeMillis() % 1_000_000    );    @Override    public long nextValue() { return counter.incrementAndGet(); }}
```
...
4. 缺失的字段补上默认值（ `defaultTimeoutMs=60000` 、 `maxReactIterations=10` 等）
...
排除的原则很简单：如果一个包的所有类都是线上专属的，用 ComponentScan 排除；如果是某个自动配置类，用 exclude 排除。不需要在每个类上加 `@Profile` 或 `@ConditionalOnProperty` `。`

**9.5 一键启动：start-local.sh**

```bash
# 一条命令启动本地环境bash scripts/start-local.sh
```
...
1. Health Check： `GET /checkpreload.htm` 返回 "success"，H2 控制台 API 能正常查表
