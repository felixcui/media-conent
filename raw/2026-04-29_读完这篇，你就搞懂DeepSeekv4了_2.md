...
**来源**: https://mp.weixin.qq.com/s/MamimcCQj_Hd12T8iFVmKg

---

## 摘要

DeepSeek‑V4 预览版以 1.6T 参数（稀疏激活 49B）和默认 1M 上下文成为“地表最强开源模型”，在编程、数学、Agent 和长文本维度接近闭源三巨头（GPT‑5.4、Claude 4.6、Gemini 3.1）；其真正硬核之处并非单纯参数与上下文规模，而是从 attention 到 kernel 的系统级重构与优化，使模型能支撑 Agent 多轮任务、整仓代码理解和长文档推理等实。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz9051NdCwgiczJ1fpJORZuau1gcNL1uwD4UNrZ8EQL6YATwCp1dxqG6HjI8PNQGiauGfPNic2UKX1ufTUOfjbeu3NZrh6uNpt1nSvWg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz9066lteWzwkMEZicpHFUtDjRwUlZE4vJs0ObIH3EEmMKMhK6mBQPgzB0qb4OjUeAqIAh1kVLfA7sf2e9lMJibrWA67vft9eTGAj18/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz906Uh6M3TP4d3NssibHbn0xxaYpXW38INXOafS4S4ibm1uTFbvnQMzeCxKFCb6CJdpWPqFsiao6EZoZKnkg4ZcGTTO2SBMFFEygXicg/640?wx_fmt=png&from=appmsg)

也就是每一层的输入隐状态=前一层的输入隐状态+前一层变换（attn或ffn层）。我们将这个公式逐层展开，同时令h1=v0，v0就是原始embedding后的结果用于模型输入；vi代表每一层的变换结果f(hi)，得到如下公式：

![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz9074wAk3NU9sGZdOldictbLrhKL0YzfAibA3lzXzJSZeRaYEdliat6tzic4WDFCYib3iaR5thHlzAuEaNKywnQwCIzRrbVTVagCnGiacT4/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz906WOc30ELfeUB13mErsg4Okb4lj4YIUKmnvctzVW3Y3Gjw4Y8mK5e2icv8keJhoVImKia3ibSGdGjfRcJFaG4Yrp4rB3B0iaYjunjg/640?wx_fmt=png&from=appmsg)

层间残差连接维护了n个流，每次层间变换（ATTN or FFN）前，先进行降流为1，变换后的结构再升流为n，进行带权重的残差加和。同时把上述公式展开成公式(2)的形式，得到如下公式：

![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz904fPOBuekY6nuWFuiatP74HPJ2CddzhU6jFMwtJPw7SX8SxmCNicZKXziahodAn17GhCgmEGrSAUbm92ib7fYXpwQgqichTdM7H8cOA/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz907Sv563pRrE44Gq7AmdzxscKp1g6aFasUa6h1glqiapuGBhBG1F4TexRpV4KudAyRMg8Pty8npluxLfaXG3ufkVibiaZ3icicBHahd8/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz907MG1WGibWCOaaewf9JicgAOQzgibYoswh1d4A28RIWibHJsibvFRia91sibWWL9rI8iax5t30fKrbPwu65jdW0tJ9TGckxia9mmktO45rE/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz907P9kn5UkYgaYt9nH6j5mCYIFoicKlUShClotjV52dgjQSjKUs7r25lKPb2ho6Dic5wZj3wLn81tNibWw9h7xTIKk2qUs8vibV6Zww/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz906NNkOGQE2WO0M2eTXkMedwrXHf7Ps1Vrwe9icRPlzHic0RK5vl4M2aslXwgsTcS5D0fvLaWoiaV0NaZ0ibp3yv3yXzw0JKqIglULs/640?wx_fmt=png&from=appmsg)

其中，公式左侧代表当前组的token的带权重加和压缩，公式右侧代表前一组token的带权重加和压缩（注意这里的计算是哈德玛积，不是矩阵乘法）。矩阵S为权重矩阵，定义为公式(7)：

![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz9052lFcMxSfDU0rhOhcOIa2AMswn8Nm2Sias8XKEvJiauj3mwVDtqUqMjia9oBiaCQeqGggDibeDuKCbw9H4Lfic3VI7YnQQ4SatJFdpw/640?wx_fmt=png&from=appmsg)

而计算上述内容需要的值投影矩阵C和矩阵Z由隐状态值投影得到：

![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz907AzngusG0fgvj6SZicwXBARZol4GrpT88acMNea2czfgLxF7sXju4eCqnr6JSMJLAjvTibOudj0ib4Jl9QhlVOticDEDrGLciakan4/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz905FiauL7abjhhtXiaR0sUfNWPlKSTVXO27BOeTUuDJNrnrmPmaqE5eunbzczDdEox3GrH6nzYxlseNQnEk1fr9qUqVNV3ia1c4ArM/640?wx_fmt=png&from=appmsg)

其中K矩阵采用和公式(6)完全一致的计算思路，其实它也代表的是压缩结果的“key投影”。每个token的查询矩阵q和注意力每个头的权重矩阵w也均由隐状态值投影得到：

![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz905loqf6NEf12w0m9TDXf4dbYyhib31icxM6KBGic4cOEujCb13fAeHbjv4RUPqIRO66GxvzXE6USZx4ovrPM4jWibvSh7T9cuoDUlQ/640?wx_fmt=png&from=appmsg)

同样，投影计算用到的矩阵也都是通过训练得到的。

**注意力计算** （对应步骤三）：这里其实就是标准注意力计算了，只是共享了KV，注意力计算方法为公式(9)：

![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz907WvhjpVoko3dibfbjdQ5MPU7hyExTNbfib1j01OViaj4jQGkhGRmUKVrUalic4zddOfCwMNYVxAmLXRdkSfxAsj0vbEic4mmwr2PYE/640?wx_fmt=png&from=appmsg)

其中KV共享的C矩阵就是闪电索引得到的top-k采样得到的K组压缩过的信息，q是每个token的查询矩阵，计算方法不再赘述。

由此，则完成了token压缩+进一步稀疏采样的CSA机制，CSA完整的架构也可以参考下图：

![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz906PWoOzgs49LE9YqbWoKwfZkqNQbs7EJibwdobCbjic3ic5JP1IziaTsBfX6libxc7wnVwjbviaWCvFWV6HV9FXoTwm93yrfiahXay1W8/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz904KDWQiaj5tgtwJicEQhM0zL7YFD5fHGDrYXia6Pc3hJqWFb3oPSBUDCCj7s5Uu4b82KRPqv4rXEeyEiahpLxpPszrzL6USqiaxfYb8/640?wx_fmt=png&from=appmsg)

可以清晰看到，HCA的压缩机制无需连续两组压缩内容的拼接，且加权计算、权重矩阵S和值投影矩阵C的计算方法与CSA中完全一致。

权重矩阵S定义为公式(11)：

![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz904ibVLHDBwaSKRQLdwibJhXuVXlYMznehHD09KDnpTWOvBibCs0SVTFibOu1dFJL9y0O3kT60jLl1bwYMhReIlLzIUynsaNT6mEBIo/640?wx_fmt=png&from=appmsg)

值投影矩阵C和矩阵Z也同样由隐状态值投影得到，且2个W矩阵和偏置矩阵B同样也都是可学习的，在训练过程中得到：

![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz906l9owPVFicHbzsuy7xibGvPyj8tXicJd1Hg6EMThhDG1uNHeekQQAsEVV3NLhHniaEAPhT46TB0xSk1mQEicOOKuAPQ7iaRyEcLTFlc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz907Q95vicso0ncHmjicF7a80RHBFdVGFl6vTjdyMdg2RcMTEHmXdVs1jK0LYhaKvQkj8scYZSTlGtmRVjWv0DqhuNiahAicO9rq3PNA/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz906upsnX78ve7HIU6ibcDmsKafJox3d4xUAGjw1R6PM74MQrIjmYQ4J2ibI8rPu6ASkZdBFSLaZibImbcy4LuhB1qFEXMm0RXZfG8E/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz904vmF3GdsiabC7WVUFcevtoKoVKE5hNjoL4x7Q8AEYSd5E2m3VEYic5aYpm3pXYzSicrul79OYk2ElW40JgbRfWvibHryPxv7krrG4/640?wx_fmt=png&from=appmsg)
