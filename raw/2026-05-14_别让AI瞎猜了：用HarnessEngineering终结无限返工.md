...
![](https://mmecoa.qpic.cn/sz_mmecoa_png/YE4xKIsXPfrCNlWYTQKDE0zWUfbiaSRAx5WPvCGm6LwMTUIZ7AC6xdib3VjrRzbTeib9FNVrkJmRic8cNdgvyzxrthaKMgYkQY3AzEvT0ibDUGDQ/640?wx_fmt=png&from=appmsg)
...
| --- | --- |
| 记住项目背景 | 写进 `AGENTS.md` 、README、docs |
...
![](https://mmecoa.qpic.cn/sz_mmecoa_jpg/YE4xKIsXPfrBgLvccrrooDzEdzUueOVpEyduNb7P55lApl55iaQN9EuOldKoYK2plNa60d9YficBiasrZ0SZeJe2V7yice58neg2SWhj7a50pZY/640?wx_fmt=jpeg&from=appmsg) ![](https://mmecoa.qpic.cn/sz_mmecoa_png/YE4xKIsXPfof1hRsxKkSy8QraqJicPbJuo5GWHicW1uJgz4VpZibR9t0HMuHWcwfcLL5WiaYWqoHRWUGxITwhTgK0KAtNqLVmg9Bho59L7zPH8U/640?wx_fmt=png&from=appmsg)
...
1. 把harness模板压缩包和目标项目路径一起交给agent。
2. 先要求agent只解读，不修改目标项目。
3. 让agent输出初始化方案：会新增什么、会修改什么、会保留什么、有哪些风险。
4. 根据目标项目的技术栈、任务系统、验证命令和团队约定，选择需要初始化的层级。
5. 确认方案后再执行初始化，并在完成后运行项目验证命令和harness检查。

模板包提供的是一套可复用的项目结构，不是一组必须原样照搬的文件。落到项目里时，agent应该先理解模板，再理解目标项目，最后再把两者对齐。

![](https://mmecoa.qpic.cn/mmecoa_png/YE4xKIsXPfqcXhqO4PlobABAKe4OjPicnrCEjoTYzRazf2SjvAStvEEsUjPLRdpdygpT1jVuOOyMV6vDnAVLr1iaRMlyahopFV1ryfibPhfGOs/640?wx_fmt=png&from=appmsg)

模板目录把入口、计划、约束、验证和脚本放到项目内，方便后续任务复用。

一个典型初始化结果大致如下：

![](https://mmecoa.qpic.cn/sz_mmecoa_jpg/YE4xKIsXPfomQCwNpVNSgYI7RYHaiauvxvZ7UhnO0Ttja0RldZC2VY5AiaxxbLfstClQYbD1xSDAXeGm3xSHmVynHp7LMk4qic0MHibFYhtC0Jk/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmecoa.qpic.cn/mmecoa_png/YE4xKIsXPfq6zFbaRRIFpzsyZKKQ1YCxibg6BHxibI6rAFIWibW1NDibQlbsW4Ysgju8WDicib9iccAyIbtic3gR46X0lFqmpBnbLC9iadSSxZl2Xjsc/640?wx_fmt=png&from=appmsg)
...
| 状态暴露层 | `Storybook` 、story文件 | 显示 `Default` 、 `Empty` 、 `Loading` 、 `Error` 、权限态、操作反馈态 |
...
![](https://mmecoa.qpic.cn/sz_mmecoa_png/YE4xKIsXPfpmtlIgzryVggDDKt0Cbnm1Yo4ypLYmzJepRqPUtaEGOnmKU9z4jmHYo2IwNYLvfIicJbNr7MYZ8RyAFWXtfcFhlYuvLHt8oNvI/640?wx_fmt=png&from=appmsg)

后端任务中， docs 、 plan 、 ter minal 、 logs 、 repo 和 review 共同组成验证反馈循环。

![](https://mmecoa.qpic.cn/mmecoa_svg/Q3auHgzwzM4CDCfqCkH8ZW8HZyWxoWjGRDPP3kicLY0kvue5OeVE7RAgyCkDrgmyvk3s8lmhQoA5mF3YUyiaqyIBdcOOh4UIvSQYx5gMMmAbuZJnLa1b8REw/640?wx_fmt=svg&from=appmsg)
...
![](https://mmecoa.qpic.cn/sz_mmecoa_png/YE4xKIsXPfoPPWu9pwp98kutVRzafGO03FB3GyA6SibfBPjlTv1WMFJ2fqcE1BabMYD7cFd8pIxqRIUmib9DgNo3HxFU1RmCsrLKX4NHuQiaf8/640?wx_fmt=png&from=appmsg)
...
| 2 | 仓库入口地图 | 根目录放一个简洁的 `AGENTS.md` 或同类文档 |
...
```markdown
你是我的研发协作 agent。我会提供一个 harness 模板压缩包，以及一个目标项目路径。请按两个阶段工作。阶段 1：只解读，不修改文件。1. 解压或读取 harness 模板包，说明模板包中的核心目录和文件职责。2. 读取目标项目的现有结构，包括 README、AGENTS.md、Makefile、package.json、go.mod、pyproject.toml、docs、scripts、CI 配置等实际存在的入口。3. 判断目标项目适合初始化哪些 harness 内容：入口地图、控制面文档、计划协议、测试 runbook、项目约束登记、agent prompts/guides、脚本检查入口等。4. 输出初始化方案，必须包含：   - 准备新增的文件   - 准备修改的文件   - 明确不覆盖或需要保留的文件   - 需要从目标项目实际情况推导的内容   - 初始化后的验证命令   - 可能的副作用和回滚方式5. 在我确认之前，不要修改目标项目。阶段 2：确认后再执行初始化。1. 按确认后的方案初始化 harness 文件。2. 保留目标项目已有的工程结构、构建方式、测试命令和团队约定，不要把模板内容机械覆盖到项目上。3. 如果发现模板假设和项目实际情况冲突，先停下来说明冲突，不要自行扩大修改范围。4. 初始化完成后，运行可用的验证命令，并给出结果摘要。5. 最后输出一份初始化报告，包括已创建/修改文件、验证结果、尚未机械化的项目约束、后续建议。
```
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/unymlvueOvvXwhLEYHkepiaFicVMpbsBLVD8UBl8EaQ4j9XwcUpjafibX1SuB6zZOJGy2VDN9gjgQa4xM9tnh3fHQ/640?wx_fmt=other&wxfrom=10005&wx_lazy=1&wx_co=1&tp=webp#imgIndex=10)
