...
**来源**: https://mp.weixin.qq.com/s/DLZFEsqxRMq0ntilgqpEjA

---

## 摘要

文章聚焦拆解 Hermes Agent 的五层架构体系，通过"跟踪一条消息从输入到输出的完整链路"的方法论，逐层揭示其内部实现机制。Hermes 在短短两个月内获得四万七千颗星，其架构从上到下分为入口层、网关层、执行层等，入口层支持 CLI 和二十多个消息平台适配器，网关层由 GatewayRunner 常驻进程管理连接与会话生命周期，执行层负责 AI 推理调度。
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/6Uzn2S5AAyTDT6Nm6YiawJEhufhl7icLUTUMLDh6AtnHe0zias0WjcsXnAjYGelFzCywRysMNMVSlpe0mjNGBFJdOrk7TEeHtt5f1lWH8dtF6A/640?wx_fmt=jpeg&from=appmsg)
...
这个过程涉及多轮工具调用（web\_search 搜新闻、web\_extract 提取详情）、信息整合、分类归纳。如果新闻来源多、数据量大，可能还需要拆分子任务并行搜集。

那 Hermes Agent 是怎么把这些事串起来的？先从整体架构说起。

![](https://mmbiz.qpic.cn/mmbiz_jpg/6Uzn2S5AAyQJNbarU1ebeuAZCRj3jWwGSXoL2JSMTwJWxp00zQa1EHoXIaQAHibWFibjXALGTXEnvLTQVvfmOG4KWlug37uaRTq9nQZgOCG3Q/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/6Uzn2S5AAyQXkBGsHytgVTU5gYpjWQR1DJDFxQgH30gkbYnn34VxUlLuYuqr5d15ia7ZF1JXGWUMDOMicgQibyblia3y25rzNhOP3Mic3PgZfWJc/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/6Uzn2S5AAyTib4dZtebRe2p7ibgYPlAsTWwO6jLwZFlDQQIh0udIrmoHibBEM0jLEIr8ybFKMjA2LMVXV4skUu2ZM0iaVIefSNwlGwlWgld3jCA/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/6Uzn2S5AAyRmkZaouibBXPFr06joUVC03fhUMxSnGWacEZso8QicLAlWVOByicWT0dAm9IPjPwr23wLr8JfYcojRm65OxibkZdBpVyWubrDNpL4/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/6Uzn2S5AAyT5jvRpwllTROe0mFc5MUUylv2JA2oTsYCtCe3a9LyEDhOO6QrYJzWtG1YP9YDAwebrb9A24BHp9nhTZ0GRpnaOMhVbGu9wxgw/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/6Uzn2S5AAyRPDjQQLdWBb8SSOqmcKXyyGKITFDWcFzJk7xeRq14xRVNsybXewfsyrbib2qKibRynt5fNnxYlzS0AhFicxERNWcLTQwzGpRmIVM/640?wx_fmt=jpeg&from=appmsg)
...
- **MEMORY.md**:Agent 自己的笔记本("这台机器 Python 是 3.11"、"这个项目用 commitlint"、"web\_extract 对这个网站不稳定")
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/6Uzn2S5AAyRUOcohkHfW2dTpJFtMrtmtWRPqQYlSCs4M8vCAXGhZQ3LFTeMZBztN8kvUsI2YjgsuOhdboibIzfQNypvNoMu3hSo5qz1aBibVU/640?wx_fmt=jpeg&from=appmsg)
...
所有"这条报错里带 rate\_limit、那条带 insufficient\_funds、还有一条是 openai 模块抛的 BadRequestError"之类的脏活儿全集中在分类器里,一次性把错误映射到恢复动作,主循环只负责 dispatch。
...
如果前面 tool\_calls 已追加但没执行,会补一个伪造的错误 tool result,保证消息结构对 API 合法,下次恢复对话不会被 Provider 拒。

这个伪造错误 tool result 非常有用，如果你做过Agent肯定知道这是个什么梗，用户中断后，如果工具执行也被中断，没有记录结果，那么下一次调用就会报错。

## 七、消息返回

![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/6Uzn2S5AAyTKzhrx2aerCWKaokDKfMktzc4ntlABLhuxJH11OgznNxToSZ6cmsZcxbgtmNCHCURF08t43298UjYJBC2zD3h7GGR1vdqxeFQ/640?wx_fmt=jpeg&from=appmsg)

模型给了最终回答,文本沿着 **和进来相反** 的方向走回去。

流式 token 通过 `_fire_stream_delta()` 边生成边推。CLI 下直接写进 prompt\_toolkit 的 `patch_stdout`,Gateway 下由 `stream_consumer.py` **按 1 秒节流编辑同一条消息** (不是每个 token 发一条,那样会被平台限流干死)。
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/6Uzn2S5AAyQH3hBTWXnibibjtYzXKozbicEX9eRCyptmnVo03YTkdPPic9RpJjn33LuRasAxiaQFPXRfy9ukzhTKWhCLx7PmvZb5ILh8WI1yicECo/640?wx_fmt=jpeg&from=appmsg)
...
> run\_conversation() is called once per user message in multi-turn sessions. Shutting down after every turn would kill the provider before the second message.
...
![](https://mmbiz.qpic.cn/mmbiz_png/6Uzn2S5AAyQpzPk2VzzOtic38dcDbEwZ5uBqO19cWW7f6OwLgVdicLicxtHs4swiaDPJoywdANcvCyR0x9nHhibCQUWMnXd34wIwLweKoB2gQd3s/640?wx_fmt=png&from=appmsg)
