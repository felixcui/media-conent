...
**来源**: https://mp.weixin.qq.com/s/Vl27JCPRcrM4rirewP_Hjw

---

## 摘要

本文分享了AI Agent驱动全流程开发中的Token成本优化实践。面对Harness工作流中Token消耗过快且去向不明的问题，团队首先通过AgentLens工具精准度量，定位出系统提示词、工具返回信息和历史消息为三大消耗源头。随后，团队确立了“按需加载、减少无关、减少重复”三个核心原则，从架构拆分、稳定前缀、渐进式披露等十个方向实施系统性改造，最终实现全流程Token成本预估下降50%至65%。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr4gu6FAj0YBM1C5uuiaYnnqpD8P4AwUKCfxKZ3ib74z1Jhp1c3DkygDpbYywZHtdNF1c9OyS6gAodhiaJlE6sZNZAOcdrmZTqdOGY/640?wx_fmt=png&from=appmsg)

**1.2 痛点分析：六类消耗来源**

一次任务的 token 消耗可以归为六类来源：

![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr6xZKuzgZTeeV4HPPmuVOibVtH26X8QF1J9QibOFicokgxOxd1gxuePhBLxSWTibvJAFrxwwVTJYNXdrB27ENHxg3tXuNmd24WDkHk/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr68zqSFSEraKiaqg1Y2oCduMGUJ4zJvNT9TWYAwafKnl24XcPYBcXsKHzgxC9USX01tyI8dV9b0IwNnTk2qhCstQ8W381b2XBDU/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr4KBhOEb23AIY0XUX0coSNDslMKxYTKaNkSushvePFxJw8eWAkjKOA2WVZvm8yqXiaVHdwbeib3hfBM6kL5Q8g8PaAia8cickBLgug/640?wx_fmt=png&from=appmsg)

**2.2 关键技术选型**

![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr7BJZLVajCEOEeweCkMM83X9yo9RSMsdNicSNYiadRAVLjiarvImL0ys3Whvtgdkaj5hw09EhJsr9f8ibTg2QHDd24ianhQeF0AeOVs/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr5tvMrhNib6gJUXn2CcmNGHzzltVgKqb6wtyWQYLbibaNpGicytjv4PAjicwjFG3ksH0q3qb5yovdU4JGUW4Z21VL3KX95tzJkyEn8/640?wx_fmt=png&from=appmsg)
...
骨架负责"决定下一步走哪"，具体"怎么走"则按需 \`read\_file\` 对应的资源文件。
...
```javascript
LLM 决策：判断调用哪个工具、构造参数         ← 一次 API 请求工具 JSON Schema 常驻 context               ← 每轮 10-15KB 额外 input工具返回结果进入 context                     ← 可能几千行LLM 处理结果：分析输出、决定下一步           ← 又一次 API 请求
```
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr5l7Evvsd1eEsV45MaJrppqezE2qyDC5pwKQcI9CPA58z8icIIp1ICAp99MAQmjvTLVvCYhka5wvIPOuvGSAQBF6iaOicoGpYMiaOw/640?wx_fmt=png&from=appmsg)
...
```bash
❌ 改前：直接扫描 team/project 下所有历史文档，命中关键词就整篇 read_file✅ 改后：  1. 先 read_file INDEX.md（几十行的标题+标签+摘要表格）  2. 按关键词匹配标题/标签/摘要，结合类别权重计算相关度  3. 取 Top 3 命中条目，才对这几篇 read_file 正文  4. INDEX 不存在时才回退到全文 search_content（兜底，非常态路径）
```
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr4LVQ2iaqJY7czAG8VxcJ8ZGSRX4tutNaNjKDsLfmm0ATDqicddx0qwtnkb4GCzbbWicqJzuru5icKMDMTETAHT47pqZ7xOZRqeUqc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr4lqBlickHdfD0QfGqjjD0x64oMXCwwRmGOxWbKMyYwzvk8xsSVZVTuOGzDcmsw2xsvrE4mWkD3PaUVVZ6lUhBEXTM0W40pKSh0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr4MTg4clbEK9BK0DBPJ3bXQ4jE1fps7Gald6NuOFJCEVLWn6oapZuIPe0yWkvQZ2icv0pVze69k05qLyn2GJ0EVdmniciaEuVibiaibk/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr7ic6qIByEFiaiazJnRfXmBNsOjqHYkfbQuhCIb06s1MLohoRK5Imj3SqicQ2awQYQfqBDp0PI3icUbxaDm0HzTelkic4wNdaAYVyBOM/640?wx_fmt=png&from=appmsg)
...
```js
❌ 改前：TAPD 摘要获取 → 等结果 → 再发起 Figma 摘要获取 → 等结果  两次子 Agent 调用互不依赖，却占用了 2 轮历史累积
✅ 改后：TAPD/Figma 若同时存在，同一轮消息内并行发起  Task 工具同时传入两个 subagent_name 调用，一轮内拿到两份摘要
```
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr7zy5hhzXJ6Y019AISP5Y2MEN1a4craPOse1jw5sxVvRic2RRiamPxFFHHsadyBickicXOeicNYJLcVnaOrXGdDtjhUiabGlzVFHrWNI/640?wx_fmt=png&from=appmsg)

**4.2 分项收益汇总**

![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr6axy86gD7b8czop9N51K0IHOibOUhN7KY0y4GoRbdiawicKlPxzNk6K6hkTcTXKZA8t5TB9DYbRLpicU02Pmmm3XkGD2wjg7iakFSA/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/ZRhjO8xAWr7icP7tCibD2oxpZnohkIuBfHacenAW7GTcyZ3jP6FqulqicFWezLcYnfwVlkLzEiaev7vY2ODGMqtiaWORXN3xwezWeoZy4NNLpIn0/640?wx_fmt=jpeg&from=appmsg)
...
你对本文内容有哪些看法？同意、反对、困惑的地方是？欢迎留言，我们将邀请作者针对性回复你的评论，欢迎评论留言补充。我们将选取1则优质的评论，送出腾讯云定制文件袋套装1个（见下图）。7月31日中午12点开奖。
