...
**来源**: https://mp.weixin.qq.com/s/cHs53TE4MvCqnyS5lao3KA
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr4VEzsUCzsOPpicm1lvdy3ArC4xpDTlC5hhAlCZvNBSeFfYe89hlqIcvmWiae4sRPO5clOjc0jbUNnU01TTdIawEuktJugzspay0/640?wx_fmt=png&from=appmsg)
...
- **全局级** （ `~/.claude/CLAUDE.md` ）：所有项目共用的规则，比如"不确定就问"、"先看代码再提问"这类与具体项目无关的行为习惯
- **项目级** （项目根目录 `CLAUDE.md` ）：只在该项目中生效的规则，比如"Proto 文件只改不生成"、"本项目使用 Bazel 构建"
...
```diff
---paths:["*_test.go","*_test.ts"]---
- 测试函数命名使用Test_功能_场景_预期结果格式- 每个测试用例必须包含setup、execute、assert三个阶段的注释分隔- mock数据统一放在testdata/目录下，不要内联
```
...
- 不要修改已有的迁移文件，只能新增- 每个迁移文件必须包含对应的回滚语句
```
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr4HCTFpsEA3jYRP0dzIHxSzqQQZ4bquWAUb5Ba7TSOh09ZvhBGSufFiaNIibReqLTHbsHuUdfuSf2AS5a6FZgrHga2B8KSewlJMc/640?wx_fmt=png&from=appmsg)
...
```javascript
bazel run //:gazelle <project_path>
```
...
- 监听 `PostToolUse` 事件，匹配 `Write` 工具 → 检测文件创建
- 监听 `PostToolUse` 事件，匹配 `Bash` 工具 → 判断是否执行了 `rm` 操作，检测文件删除
...
```bash
#!/bin/bash# PostToolUse hook for Bash tool: run gazelle when a file is deleted under目标目录set -euo pipefail# 从 stdin 读取 Hook 传入的 JSON 数据INPUT=$(cat)# 提取执行的命令内容COMMAND=$(echo "$INPUT" | tr -d '\n' | sed -n 's/.*"command"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p')if [ -z "$COMMAND" ]; then  exit 0fi# 判断命令是否包含 rm，不包含则直接跳过if ! echo "$COMMAND" | grep -q "rm"; then  exit 0fi# 判断操作目标是否在需要监听的目录下if ! echo "$COMMAND" | grep -q "<your_project_path>"; then  exit 0fi# 确认命令是针对目标目录的 rm 操作，执行 gazelle 更新 BUILD 文件echo "File deletion detected in <your_project_path>"echo "Running gazelle..."bazel run //:gazelle <your_project_path>
```
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr5A7OfY1ul9bzfLUAUrWFeRpUrhO1xvLYVzicoAaIrFO6wA41tUic66f29Q4Z9ubeB90lVeCVb0pzZjJHd7pQpFzUEB68ibqcw1Fc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr4P8AKamSYmwI6iajQakcTUVIoxgGrRsRJRtLjpNRib38fGE7nKanl335Z4ntGLklhCanvfVPx6RDcIuJ8rrLe8E2tXwXSKb9LeM/640?wx_fmt=png&from=appmsg)
...
```shell
# 在主会话中，完成接口开发后> /branch
# 进入分支会话，此时拥有主会话的全部上下文> 请根据刚才开发的接口，生成 Apifox 格式的接口协议文档
# Claude 生成文档（依赖之前的上下文，知道接口的参数、返回值等）# 复制文档内容到 Apifox 后，直接关闭分支会话即可# 主会话不受任何影响，上下文保持干净
```

简单总结一下两者的区分：

![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr7V3UfuS3jyuPQaYN8WpuHfUyODRgPj0u9kiaWTUQdYdJPnh375HPWS75PdHNWMTrCkjLIXaK1h6VAlsjw6Uic0F4uqXNa9XXVhA/640?wx_fmt=png&from=appmsg)
...
```markdown
---description: 查看未提交的代码变更，生成 git commit 命令disable-model-invocation: truecontext: forkagent: Explore---
## 代码变更状态
!\`git status\`
## 变更内容
!\`git diff\`
## 任务
根据上面的变更内容，用一句非常简短的中文总结变更摘要（不超过20个字），然后只输出以下格式的命令文本，不要输出其他任何内容：
\\`\`\`git add . && git commit -m "feat(xxxxx-web-go):摘要内容"\\\`\`\`
```

几个关键配置说明：

- `context: fork` ：以子代理的方式运行， `git diff` 的大段输出和分析过程不会污染主会话
...
```markdown
---description: 审查当前分支的最近改动---
请查看当前分支名、最近 10 条提交记录、以及与 main 分支的 diff 统计，然后审查最近的代码改动，指出潜在问题。
```
...
- **工具层面的最小化传递** ：读取文件时可以指定行范围（不必读整个文件），编辑文件时只传递差异部分（old\_string → new\_string），而不是传整个文件内容
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr6YY4woQrlzRmibfmMMpP6Re7UQbL2QWxesPypA9AxxvrmiaJTO3RJVclia3EMUZJeCa99JC5JxBmW2Idq1ZjlH2t01tDyVia56Gto/640?wx_fmt=png&from=appmsg)
...
```bash
# 1. 添加插件市场源/plugin marketplace add git@git.xxx.com:xxxx/xxxxxx.git
# 2. 安装插件包（包含所有 Hook、MCP、Skill）/plugin install <插件名>@<市场名>
```
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr7yj0AGM5VsQJIwex2u6NfwZZSQCeVMZNOynbjS19mn4jicuJqFclp188DlVcdcaAia8eozRwSsK4LRCfKKbRS8Cwhz01oKMOPrs/640?wx_fmt=png&from=appmsg)

安装后，右键选中代码即可看到 **Send to Claude Code** 选项，直接将代码片段发送到 Claude Code 的会话中。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr7o4h56ecktJ5gnOoB8NXZVVAQicxKueRCckibzjzMZLWicKMTX5elQSu5cy1rBEzfPBI41TRAoPvwo3ib1GMFPJricSE6jxURJumng/640?wx_fmt=png&from=appmsg)
...
你对本文内容有哪些看法？同意、反对、困惑的地方是？欢迎留言，我们将邀请作者针对性回复你的评论，欢迎评论留言补充。我们将选取1则优质的评论，送出腾讯云定制文件袋套装1个（见下图）。8月12日中午12点开奖。
