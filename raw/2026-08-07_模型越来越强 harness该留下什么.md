...
source: "腾讯云开发者"
url: "https://mp.weixin.qq.com/s?__biz=MzI2NDU4OTExOQ==&mid=2247696815&idx=1&sn=273c44cd529f4ae4cac7328bb19e54b8"
author: "魏依承"
date: "2026-08-07"
summary: "文章指出，随着模型能力提升，旧式harness中的冗余指令、过度验证和脚手架已从"无害"变为"负担"，应尽量删减并改为判据式表达、按需加载skill、用可执行规格替代文字说明。团队自建harness的价值，转向保留真正不变量、沉淀业务约束与工具接口设计。"
---
...
- The new rules of context engineering for Claude 5 generation models，Thariq Shihipar，2026-07-24 —— 删掉 80% 系统提示词、六个转变、规则→判据的 before/after 对照
	（https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models%EF%BC%89
- Prompting Claude Opus 5（Claude Platform Docs）—— 验证指令要删除而非改写、字面执行导致召回下降、legacy harness scaffolding
	（https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5%EF%BC%89
- Prompting best practices（Claude Platform Docs）—— 过度提示导致过度触发、CRITICAL 措辞降级、prefill 从 API 移除
	（https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices%EF%BC%89
- Agent Harness Design: 3 Patterns for Harnessing Claude's Intelligence，2026-04-02 —— 三个设计模式、what stays / what goes
	（https://claude.com/blog/harnessing-claudes-intelligence%EF%BC%89
- Harness design for long-running application development，2026-03-24 —— 「每个组件都编码了一个假设」、context resets 成为 dead weight、evaluator 价值边界外移
	（https://www.anthropic.com/engineering/harness-design-long-running-apps%EF%BC%89
- A harness for every task: dynamic workflows in Claude Code，2026-06-02 —— agentic laziness / self-preferential bias / goal drift
	（https://claude.com/blog/a-harness-for-every-task-dynamic-workflows-in-claude-code%EF%BC%89
- Lessons from building Claude Code: How we use skills，2026-06-03 —— 复述默认行为的 skill 零价值、验证类 skill 影响最大
	（https://claude.com/blog/lessons-from-building-claude-code-how-we-use-skills%EF%BC%89
- Skill authoring best practices（Claude Platform Docs）—— 上下文窗口是公共资源、自由度分级、先建评测
	（https://platform.claude.com/docs/en/agents-and-tools/agent-skills/best-practices%EF%BC%89
- Effective context engineering for AI agents，2025-09-29 —— 注意力预算、context rot
	（https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents%EF%BC%89
...
- Harness engineering: leveraging Codex in an agent-first world，2026-02-11 —— 强制不变量而非微管理实现、AGENTS.md 约 100 行的「地图」、用 linter 与 structural test 强制
	（https://openai.com/index/harness-engineering/%EF%BC%89
- GPT-5.5 prompt guidance（OpenAI 开发者文档）—— 删除分步流程指引、过度指定缩小搜索空间、真正的不变量 vs 判断题、从最小 prompt 重新起步
	（https://developers.openai.com/api/docs/guides/prompt-guidance?model=gpt-5.5%EF%BC%89

Addy Osmani（Google Chrome 团队工程主管，个人博客）

- Agent Harness Engineering，2026-04-19 —— harness 不是缩小而是迁移、棘轮法
	（https://addyosmani.com/blog/agent-harness-engineering/%EF%BC%89
- Own the Outer Loop，2026-07-15 —— 只有人能继承后果、Verdict / Answerability / Accountability
	（https://addyosmani.com/blog/own-the-outer-loop/%EF%BC%89
- Earning taste and judgment，2026-07-14 —— 监督者悖论
	（https://addyosmani.com/blog/earning-judgment/%EF%BC%89
- Software Factories, Light and Dark，2026-07-20 —— back pressure、验证才是瓶颈
	（https://addyosmani.com/blog/software-factories/%EF%BC%89
- Agentic Autonomy Levels，2026-07-02 —— 六级自主度、三个判定问题、四个反模式
	（https://addyosmani.com/blog/agentic-autonomy-levels/%EF%BC%89
- The Intent Debt，2026-06-05 —— 技术债 / 认知债 / 意图债的区分
	（https://addyosmani.com/blog/intent-debt/%EF%BC%89
- Stop Using /init for AGENTS.md，2026-02-23 —— 能找到就删、第二个事实来源、还没修的摩擦
	（https://addyosmani.com/blog/agents-md/%EF%BC%89
- Agent Skills，2026-05-03 —— 流程优先于散文
	（https://addyosmani.com/blog/agent-skills/%EF%BC%89
- The Orchestration Tax，2026-05-24 —— 启动便宜、收口昂贵
	（https://addyosmani.com/blog/orchestration-tax/%EF%BC%89
- Comprehension Debt，2026-03-14 —— 认知债滋生虚假信心
	（https://addyosmani.com/blog/comprehension-debt/%EF%BC%89
...
扫码领取腾讯云开发者专属服务器代金券！腾讯技术人原创集 · 目录
