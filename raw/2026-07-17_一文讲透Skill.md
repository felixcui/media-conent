...
**来源**: https://mp.weixin.qq.com/s/0Cc2pjv6Mnf6U-C-H7RWpg
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ZRhjO8xAWr59iaDPkSDwJsRXhvPjnBCgjpgmf62dAanwOckULhAbiaFfU3Z41Z5AnBNhL280Utm81zUa5lD1rNDHJaiaCok4dKE4OnRTyKsbyo/640?wx_fmt=jpeg)
...
- 行动： 整理最终报告，输出给用户。
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr6LicG8EeMUJK606Xy4yRFkXTGJpEVawTo8iaia64z7JbZkE5uqZicYFudUQ3P9pZxLt1bh1hiclzfBr5BAiaRXAfLF0q3xAAiaamOqtY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr66N7Z54qNOqgXD3yUr0wJ2siaJ55kzdg906NvRBTIplW5U7R2SVb4LiaS4IdkKZianG7xQM0h4icYAW77zIZ5UODdCrYXUAicjjen8/640?wx_fmt=png&from=appmsg)
...
- AI 的思考： “我的训练数据截止到 2023 年，我不知道‘今天’的数据。但我看到手里有个工具叫 get\_weather，描述说是查实时的。所以我必须调用它，否则我只能瞎编。”
...
- 场景： 用户说“帮我给老板发个邮件”。
- AI 的思考： “生成文本是我的强项，但我无法在这个聊天窗口里把邮件真的发出去。必须调用 send\_email 工具才能产生实际的‘副作用’（Side Effect）。”
...
解决方案： 给 Agent 默认只装 1 个核心技能，就是 find\_skill。当用户提问时，Agent 先用这个技能去“图书馆”找书，找到后再读。
...
- 系统找到了 google\_translate (匹配度 0.98)
- 系统找到了 text\_summarizer (匹配度 0.95)
7. 注入 (Injection)： 系统把这两个工具的 JSON 定义，临时插入到当前的对话 Prompt 中。
8. AI (大脑) 觉醒： “懂了！我现在会翻译和写摘要了。”
9. 执行： AI 依次调用这两个新获得的技能，完成任务。
10. 遗忘 (Reset)： 任务结束后，为了节省内存，系统把这两个工具从 Prompt 中移除。
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr5ura4osQDLFTFymlGiaz01erGH4e4peicvbYroOYh4szo0DeYKPenCIB2Rx81IteW1WZ31vicrTTMH4UpOyqHCjXj7Fmsl6TK1Fo/640?wx_fmt=png&from=appmsg)
...
```python
def run_agent():    goal = "做一份完美的晨报"    tools = [search_tool, summarize_tool, ppt_tool]  # 工具箱
    while True:        # 1. AI 观察现状        status = check_status()
        # 2. AI 自己决定下一步用什么工具        action = llm.think(goal, status, tools)
        # 3. 执行 AI 决定的动作        if action == "search":            result = search_tool()        elif action == "retry_search":  # AI 发现第一次搜的不行，决定重搜            result = search_tool(keyword="换个词")        elif action == "finish":            break
```
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr69tVS6kGfu2Wcpt8lBQfCx6Wh0Zmlu9nibWzib3srer8vwdQFLD3f0FueshEFONAo9W7ojancEZN5Gz0O5RYbsMHaxjYW2ycr4o/640?wx_fmt=png&from=appmsg)
...
```bash
ai-morning-brief/├── config.yaml                    # 配置文件├── main.py                        # 主程序入口├── requirements.txt               # Python 依赖├── src/skills/morning_brief/      # Skill 核心代码│   ├── __init__.py                # 双模式自动切换│   ├── agent_mode_helpers.py      # 智能体模式：脚本只做手脚│   ├── searcher.py                # 新闻搜索引擎│   ├── brain.py                   # LLM 分析引擎（Skill 模式用）│   ├── ppt_generator.py           # PPT 生成器│   ├── word_generator.py          # Word 生成器│   ├── notifier.py                # 邮件推送│   ├── memory.py                  # 历史数据持久化│   ├── models.py                  # 数据模型│   └── config_loader.py           # 配置加载器├── data/output/                   # 生成的文档├── data/history/                  # 历史数据└── SKILL.md                       # Skill 说明书
```
...
```shell
---name: ai-morning-briefdescription: 搜索新闻、生成早报、新闻分析总结。零 API Token 配置。---
# AI Morning Brief Skill
## 工作流程
### Step 1: 搜索新闻（脚本执行）运行 search_news(topic='AI') 获取新闻列表
### Step 2: 打分筛选（你自己思考，不调用任何 API！）拿到新闻后，你自己打分，淘汰 score < 60 的
### Step 3: 抓取完整正文（脚本执行）对入选新闻调用 fetch_full_content(urls)
### Step 4: 深度分析（你自己思考）结合完整正文，生成 summary_deep 和 summary_public
### Step 5: 在对话中输出公众号草稿让用户第一时间阅读，无需等文件生成
### Step 6: 生成文档（脚本执行）调用 generate_documents_from_file() 生成 PPT + Word
### Step 7: 邮件推送（可选，脚本执行）
### Step 8: 汇报结果
```

搜索模块 searcher.py ，给 AI 装上眼睛；智能打分模块，AI 自己当编辑；深度分析，从摘要到洞察；自动生成文档模块 ppt\_generator.py，生成PPT + Word；
...
```nginx
# 8:00 AI 早报python main.py --topic "AI"# 8:30 半导体早报python main.py --topic "半导体"# 9:00 新能源早报 python main.py --topic "新能源"
```
...
```bash
export EMAIL_SENDER="your_email@qq.com"export EMAIL_PASSWORD="your_smtp_auth_code"   # 注意是授权码，不是密码export EMAIL_RECIPIENT="team@company.com"export EMAIL_SMTP_HOST="smtp.qq.com"
```
...
- OPENAI\_API\_KEY= (填这里)
- GOOGLE\_SEARCH\_API\_KEY= (填这里)
- TWITTER\_API\_KEY= (填这里)
- ...
...
![](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr7Sg3kziaWyRnmm24lcojBdVVXXcty75ag3b7cT11OOUXGctURiaibXXCZklfCABKOiag1fiahxlAQGGUNibGdznXup6K7EIAy02GicTk/640?wx_fmt=png&from=appmsg)
...
你对本文内容有哪些看法？同意、反对、困惑的地方是？欢迎留言，我们将邀请作者针对性回复你的评论，欢迎评论留言补充。我们将选取1则优质的评论，送出腾讯云定制文件袋套装1个（见下图）。7月23日中午12点开奖。
